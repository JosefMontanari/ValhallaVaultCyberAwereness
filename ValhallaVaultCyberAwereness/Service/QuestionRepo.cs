...
namespace ValhallaVaultCyberAwereness.Service
{
    ...
    public class QuestionRepo(ApplicationDbContext context)
    {
        ...
        public async Task<List<Question>> GetSegmentQuestionAsync(int z)
        {
            var questionSegment = await context.Questions
                .Include(x => x.Segment)
                .ThenInclude(c => c.Category).Where(x => x.SegmentId == z)
                .ToListAsync();
